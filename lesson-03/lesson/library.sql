...
SELECT * FROM book;
UPDATE book SET author_last_name = 'Scot' WHERE id IN(12, 22);
SELECT COUNT(*) FROM book WHERE author_prizes IS NOT NULL;
SELECT * FROM book WHERE sheets_qty = (SELECT MIN(sheets_qty) FROM book);
SELECT MAX(sheets_qty) FROM book;
SELECT AVG(price) FROM book;
SELECT COUNT(*) FROM book WHERE category = 'novel';
SELECT MIN(price) FROM book;
SELECT * FROM book WHERE price = (SELECT MIN(price) FROM book);
SELECT book_name, ROUND(price, 4) FROM book ORDER BY book_name; 
SELECT * FROM book ORDER BY date_of_publication LIMIT 5, 5;
SELECT * FROM book ORDER BY date_of_publication LIMIT 10, 6;
SELECT * FROM book WHERE book_name LIKE '%oo%';
SELECT * FROM book WHERE id LIKE '_2';
SELECT * FROM book WHERE author_email LIKE '%mail.ru' ORDER BY book_name;
...
DELETE FROM book WHERE book_name = 'Oroonoko';
SET SQL_SAFE_UPDATES = 0;
DELETE FROM book WHERE isbn LIKE '%6';
